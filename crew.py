...
if __name__ == "__main__":
    ...
    def kickoff_with_quota_message(crew_obj, inputs):
        try:
            return crew_obj.marketingcrew().kickoff(inputs=inputs)
        except Exception as e:
            s = str(e)
            if 'RESOURCE_EXHAUSTED' in s or 'Quota exceeded' in s:
                print("ERROR: Google Gemini quota exhausted")
                print("  - Retry later when quota resets.")
                print("Exiting.")
                sys.exit(1)
            else:
                raise
    ...
    crew = TheMarketingCrew()
    kickoff_with_quota_message(crew, inputs)
    print("Marketing crew kickoff finished.")